# Authors and Citation

## Authors

- **[Robert Gilmore](https://github.com/grabear)**. Author, maintainer.

- **[Shaurita Hutchins](https://www.shauritahutchins.com)**. Author.

## Citation

Source:
[`DESCRIPTION`](https://github.com/vallenderlab/JournalAnalysis/blob/HEAD/DESCRIPTION)

Gilmore R, Hutchins S (2026). *JournalAnalysis: Explore and Identify
Journals to Publish in.*. R package version 0.2.0,
<https://github.com/vallenderlab/JournalAnalysis>.

    @Manual{,
      title = {JournalAnalysis: Explore and Identify Journals to Publish in.},
      author = {Robert Gilmore and Shaurita Hutchins},
      year = {2026},
      note = {R package version 0.2.0},
      url = {https://github.com/vallenderlab/JournalAnalysis},
    }
